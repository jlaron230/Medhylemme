...
const formik = useFormik({
  ...
  validationSchema, // Utiliser un schéma de validation pour valider les champs
  onSubmit: async (values, { setSubmitting, resetForm, setFieldError }) => {
    try {
      await register(values); // Appel de la fonction d'enregistrement avec les valeurs du formulaire
  ...
  const handleReappearanceAnimationY = () => {
    ...
    return (
        <>
          <section className="relative z-10 overflow-hidden py-20 dark:bg-dark lg:py-[120px] bg-black text-white p-12">
              ...
              <div className="-mx-4 flex flex-wrap lg:justify-center">
                <div className="w-full px-4 lg:w-1/2 xl:w-6/12 md:justify-center md:flex">
                  <div className="mb-12 max-w-[570px] lg:mb-0 appear-animation">
                    ...
                    Chez Medhylilemme, chaque message compte et contribue à enrichir notre communauté. Vous hésitez encore ? N'attendez plus, saisissez cette opportunité pour nous dire ce que vous avez sur le cœur.

                    Et pour nos lectrices les plus charmantes, qui ne sont pas des zombies ambulants, n'oubliez pas de nous envoyer votre Snapchat par email. On promet de répondre... et peut-être même de partager un dilemme ou deux !
                    </p>
                ...
                <div className="w-full px-4 lg:w-1/2 xl:w-5/12 ">
                  <div className="relative rounded-lg bg-white p-8 shadow-lg dark:bg-dark-2 sm:p-12 appear-animationY">
                  <form className="text-black" onSubmit={formik.handleSubmit}>
  {/* Affiche un message d'erreur si le champ 'name' a été touché et qu'il y a une erreur */}
  {formik.touched.name && formik.errors.name && <small className="error text-red">{formik.errors.name}</small>}
  ...
  {/* Affiche un message d'erreur si le champ 'email' a été touché et qu'il y a une erreur */}
  ...
  {/* Affiche un message d'erreur si le champ 'phone' a été touché et qu'il y a une erreur */}
  ...
  {/* Affiche un message d'erreur si le champ 'message' a été touché et qu'il y a une erreur */}
  {formik.touched.message && formik.errors.message && <small className="error">{formik.errors.message}</small>}
  ...
  <div>
    ...
    <button
      type="submit"
      disabled={!formik.isValid || formik.isSubmitting} // Désactivé si le formulaire n'est pas valide ou est en cours de soumission
      className="btn text-black w-full rounded border border-primary bg-primary p-3 transition hover:bg-opacity-90"
    >
      Envoyer le message
    </button>
  </div>
...
const ContactTextArea = ({ rows, name, placeholder, value, onChange, onBlur }) => {
  return (
      <div className="mb-4"> {/* Div avec une marge inférieure */}
          {/* Zone de texte multiligne */}
          <textarea
              rows={rows} // Nombre de lignes
              name={name} // Nom du champ
              placeholder={placeholder} // Placeholder
              value={value} // Valeur du champ
              onChange={onChange} // Gestionnaire de changement
              onBlur={onBlur} // Gestionnaire de perte de focus
              className="w-full p-3 border border-gray-300 rounded" // Classes CSS pour le style
          />
      </div>
  );
};
...
const ContactInputBox = ({ type, name, placeholder, value, onChange, onBlur }) => {
  return (
      <div className="mb-4"> {/* Div avec une marge inférieure */}
          {/* Champ de saisie */}
          <input
              type={type} // Type de champ (text, email, etc.)
              name={name} // Nom du champ
              placeholder={placeholder} // Placeholder
              value={value} // Valeur du champ
              onChange={onChange} // Gestionnaire de changement
